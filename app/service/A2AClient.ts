...
export class A2AClient {
  ...
  constructor(baseUrl: string, fetchImpl: typeof fetch = globalThis.fetch) {
    ...
    this.fetchImpl = fetchImpl.bind(globalThis); // Bind fetch to the global context
  }
  ...
  private async _handleJsonResponse<Res extends JSONRPCResponse>( // Takes full Response type
    ...
    // Return type is now the 'result' property of Res, excluding null and undefined
    ...
    try {
      if (!response.ok) {
        ...
        // If not a JSON RPC error, throw enhanced HTTP error with status code and text
        throw new RpcError(
          response.status,
      ...
      if (jsonResponse.result === undefined || jsonResponse.result === null) {
        throw new RpcError(
          -32603,
          "JSON-RPC response missing required 'result' field or result is null."
        );
      }
  ...
  private async *_handleStreamingResponse<StreamRes extends JSONRPCResponse>( // Takes full Response type
    ...
    // Yield type is now the 'result' property of StreamRes, excluding null and undefined
            ...
            if (dataLine) {
              ...
              try {
                ...
                if (parsedData.error) {
                  ...
                  yield parsedData.result as NonNullable<StreamRes["result"]>;
                } else {
